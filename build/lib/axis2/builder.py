...
def _derive_action_modifiers(axis1_card: Axis1Card, game_state: GameState) -> List[ActionModifier]:
    """
    Handle effects like:
    ...
    for effect in static_effects_rules.derive_static_effects(axis1_card, game_state):
...
def _derive_action_replacements(axis1_card: Axis1Card, game_state: GameState) -> List[ActionReplacement]:
    """
    ...
    rep_effects = replacement_rules.derive_replacement_effects(axis1_card, game_state)
    for eff in rep_effects:
        replacements.append(
            ActionReplacement(
                type=eff.get("type", ""),
...
def _derive_action_preventions(axis1_card: Axis1Card, game_state: GameState) -> List[ActionPrevention]:
    """
...
def _derive_mandatory_actions(axis1_card: Axis1Card, game_state: GameState) -> List[MandatoryAction]:
    """
...
class Axis2Builder:
    ...
    def build(axis1_card: Axis1Card, game_state: GameState) -> Axis2Card:
        ...
        # 6. Action replacements, preventions, modifiers (structural hooks)
        action_replacements = _derive_action_replacements(axis1_card, game_state)
        action_preventions = _derive_action_preventions(axis1_card, game_state)
        action_modifiers = _derive_action_modifiers(axis1_card, game_state)

        # 7. Mandatory actions (e.g. "must attack if able")
        mandatory_actions = _derive_mandatory_actions(axis1_card, game_state)

        # 8. Choice constraints (e.g. distinct modes for modal spells)
        choice_constraints = _derive_choice_constraints(axis1_card)

        # 9. Limits (per turn, commander tax, spell-per-turn, land-per-turn)
        limits = _derive_limits(axis1_card, game_state)

        # 10. Visibility constraints (face-down, hidden zones, randomization)
        visibility_constraints = _derive_visibility_constraints(axis1_card)

        # 11. Keywords (from oracle text)
        keywords = keywords_rules.derive_keyword_abilities(axis1_card, game_state)

        # 12. Static continuous effects on this card (for Axis3 layers)
        static_effects = static_effects_rules.derive_static_effects(axis1_card, game_state)

        # 13. Replacement/prevention effects at the card level
        replacement_effects = replacement_rules.derive_replacement_effects(axis1_card, game_state)

        # 13b. ETB replacement effects ("As this enters...", "This enters with...", etc.)
        oracle = axis1_card.faces[0].oracle_text or ""
