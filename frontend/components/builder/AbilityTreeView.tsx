        ...
        <div>
          ...
          <div className="space-y-4 ml-4">
            {triggeredAbilities.map((ability) => (
              <div key={ability.id} className="border-l-2 border-slate-600 pl-4 space-y-2">
                ...
                {ability.condition && (
                  <div className="ml-4 border-l-2 border-slate-600 pl-3">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-sm text-slate-300">{formatCondition(ability.condition)}</span>
                    </div>
                  </div>
                )}
                <div className="ml-4 border-l-2 border-amber-500 pl-3 space-y-1">
                  <span className="text-xs text-amber-400">Effects:</span>
            ...
            {activatedAbilities.map((ability) => (
              <div key={ability.id} className="border-l-2 border-slate-600 pl-4 space-y-2">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-white font-mono">{ability.cost}</span>
                </div>
                <div className="ml-4 border-l-2 border-amber-500 pl-3">
                  <span className="text-xs text-amber-400">Effect:</span>
                  <div className="text-sm text-slate-200 mt-1">{formatEffect(ability.effect)}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            {staticAbilities.map((ability) => (
              <div key={ability.id} className="border-l-2 border-slate-600 pl-4 space-y-2">
                ...
                <div className="ml-4 border-l-2 border-green-500 pl-3">
                  <span className="text-sm text-slate-200">{ability.effect}</span>
                </div>
              </div>
            ))}
          </div>
            ...
            {keywords.map((keyword) => (
              <div key={keyword.id} className="border-l-2 border-amber-500 pl-4">
                <div className="flex items-center gap-2 flex-wrap">
                  ...
                  {keyword.cost && (
                    <span className="text-xs text-slate-400 font-mono">({keyword.cost})</span>
                  )}
                  {keyword.number !== undefined && (
                    <span className="text-xs text-slate-400">{keyword.number}</span>
                  )}
                  {keyword.lifeCost !== undefined && (
                    <span className="text-xs text-slate-400">Pay {keyword.lifeCost} life</span>
                  )}
                  {keyword.sacrificeCost && (
                    <span className="text-xs text-slate-400">Sacrifice</span>
                  )}
                </div>
              </div>
            ))}
