...
export function CardTypeBreakdown({ cards }: CardTypeBreakdownProps) {
  ...
  return (
    <div className="space-y-2">
      <h3 className="text-lg font-semibold text-[color:var(--theme-text-primary)]">Card Types</h3>
      <div className="space-y-2">
          ...
          return (
            <div key={item.type} className="space-y-1">
              <div className="flex items-center justify-between text-sm">
                <span className="text-[color:var(--theme-text-secondary)]">{item.type}</span>
                <span className="text-[color:var(--theme-text-secondary)]">{item.count} ({percentage.toFixed(1)}%)</span>
              </div>
              <div className="w-full bg-[color:var(--theme-card-hover)] rounded-full h-2">
                <div
      ...
      {typeData.length === 0 && (
        <p className="text-[color:var(--theme-text-secondary)] text-sm">No cards to analyze</p>
      )}
    </div>
  );
}
