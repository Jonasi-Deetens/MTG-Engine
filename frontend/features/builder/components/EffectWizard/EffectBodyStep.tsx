...
export function EffectBodyStep(props: EffectBodyStepProps) {
  if (props.effect.effect.kind !== 'one_shot') {
    return (
  ...
  return <OneShotEffectBodyForm {...props} />;
}

function OneShotEffectBodyForm({ effect, onChange, previousSteps = [] }: EffectBodyStepProps) {
  const action = effect.effect.action as Record<string, any>;
  const effectType = action.type ?? 'damage';
  const selectedEffectType = getEffectTypeOption(effectType);
  ...
  useEffect(() => {
    ...
    // eslint-disable-next-line react-hooks/exhaustive-deps -- defaults only when type/fields missing
      ...
      {(selectedEffectType?.requiresAmount ||
        ...
        <div
          className={`grid gap-3 ${
            selectedEffectType?.requiresAmount && selectedEffectType?.requiresTarget
              ? 'grid-cols-2'
              : 'grid-cols-1'
          }`}
        >
          ...
          {(selectedEffectType?.requiresTarget || selectedEffectType?.requiresUntapTarget) && (
            <div>
              <label className="text-xs text-[color:var(--theme-text-secondary)] uppercase tracking-wide">
                Target
              </label>
