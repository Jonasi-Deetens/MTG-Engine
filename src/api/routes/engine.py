...
from db.models import CardEffectGraph, GameSession, User
...
def _attach_effect_graph_to_object(game_state: GameState, object_id: str, effect_graph) -> None:
    ...
    from engine.effects import register_active_effects_from_object

    register_active_effects_from_object(game_state, obj)


def _hydrate_effect_graphs_from_db(db: Session, user_id: int, game_state: GameState) -> int:
    """Attach saved builder graphs to objects by card_id. Returns count hydrated."""
    card_ids = {
        getattr(obj, "card_id", None)
        for obj in game_state.objects.values()
        if getattr(obj, "card_id", None)
    }
    if not card_ids:
        return 0

    rows = (
        db.query(CardEffectGraph)
        .filter(
            CardEffectGraph.user_id == user_id,
            CardEffectGraph.card_id.in_(list(card_ids)),
        )
        .all()
    )
    by_card = {row.card_id: row.effect_graph_json for row in rows}
    hydrated = 0
    for obj in game_state.objects.values():
        card_id = getattr(obj, "card_id", None)
        if not card_id or card_id not in by_card:
            continue
        graph_data = by_card[card_id]
        if obj.effect_graphs:
            continue
        obj.effect_graphs = [graph_data]
        obj.base_effect_graphs = [graph_data]
        hydrated += 1
...
def create_game_session(
    ...
    game_state = _build_game_state(snapshot)
    _hydrate_effect_graphs_from_db(db, user.id, game_state)
