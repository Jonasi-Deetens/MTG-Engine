...
def derive_static_effects(axis1_card: Axis1Card, game_state: "GameState") -> List[ContinuousEffect]:
    ...
    # Process oracle text line-by-line
    for line in text.split("\n"):
        ...
        # Skip activated abilities: "{COST}: effect" 
        ...
        # ------------------------------------------------------------
        for m in PT_MOD_PATTERN.finditer(line):
        ...
        for m in BASE_PT_PATTERN.finditer(line):
            pt = m.group("pt")
            p, t = pt.split("/")
            effects.append(
                ContinuousEffect(
        ...
        for m in PT_EQUAL_PATTERN.finditer(line):
            effects.append(
                ContinuousEffect(
        ...
        # ------------------------------------------------------------
        for m in GRANT_ABILITY_PATTERN.finditer(line):
            effects.append(
                ContinuousEffect(
                    kind="grant_ability",
                    subject=m.group("subject"),
        ...
        for m in SELF_HAS_ABILITY_PATTERN.finditer(line):
            effects.append(
                ContinuousEffect(
                    kind="grant_ability",
                    subject="self",
        ...
        # Ability removal
        for m in LOSE_ABILITY_PATTERN.finditer(line):
        ...
        # ------------------------------------------------------------
        if m := COST_INCREASE_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
                    kind="cost_increase",
        ...
        if m := COST_REDUCTION_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
                    kind="cost_reduction",
                    subject="your_spells",
        ...
        # ------------------------------------------------------------
        for m in TYPE_ADD_IN_ADDITION_PATTERN.finditer(line):
            subject = m.group("subject").strip()
        ...
        for m in TYPE_CHANGE_PATTERN.finditer(line):
            subject = m.group("subject").strip()
            types = [t.strip() for t in m.group("types").split(",")]
            effects.append(
                ContinuousEffect(
        ...
        # ------------------------------------------------------------
        for m in COLOR_CHANGE_PATTERN.finditer(line):
            subject = m.group("subject").strip()
        ...
        if m := SELF_COLOR_PATTERN.search(line):
        ...
        # Rule Modifications
        # ------------------------------------------------------------
        if CANT_GAIN_LIFE_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
                    kind="cant_gain_life",
                    subject="players",
        ...
        if CANT_ATTACK_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
                    kind="cant_attack",
                    subject="creatures",
        ...
        if CANT_BLOCK_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
                    kind="cant_block",
                    subject="creatures",
        ...
        if ONE_SPELL_PER_TURN_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
        ...
        if SORCERY_SPEED_CAST_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
        ...
        # ------------------------------------------------------------
        for m in ATTACK_EACH_COMBAT_PATTERN.finditer(line):
            subject = m.group("subject").strip()
            effects.append(
                ContinuousEffect(
        ...
        for m in BLOCK_EACH_COMBAT_PATTERN.finditer(line):
            subject = m.group("subject").strip()
            effects.append(
                ContinuousEffect(
        ...
        # Damage prevention
        # ------------------------------------------------------------
        for m in PREVENT_DAMAGE_PATTERN.finditer(line):
            subject = m.group("subject").strip()
        ...
        # ------------------------------------------------------------
        if YOU_MAY_LOOK_AT_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
        ...
        if PLAY_FROM_GRAVEYARD_PATTERN.search(line):
            effects.append(
                ContinuousEffect(
        ...
        # ------------------------------------------------------------
        for m in ETB_TAPPED_PATTERN.finditer(line):
