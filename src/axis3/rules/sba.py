...
def _apply_dies_replacements(game_state, rt_obj):
    """
    Apply Axis2 replacement effects for 'dies' events.
    Returns the final destination zone.
    """
    for eff in rt_obj.axis2.replacement_effects:
        event = getattr(eff, "event", None)
        replacement = getattr(eff, "replacement", None)

        # Axis2 uses event="die" for dies replacement effects
        if event == "die" and replacement == "exile":
            return Zone.EXILE
        ...
        for obj_id, rt_obj in list(game_state.objects.items()):
            if rt_obj.zone == Zone.BATTLEFIELD and "Creature" in rt_obj.characteristics.types:
                ...
                if rt_obj.damage >= toughness:
                    ...
                    dest = _apply_dies_replacements(game_state, rt_obj)

                    if dest == Zone.EXILE:
                        rt_obj.zone = Zone.EXILE
                        game_state.players[rt_obj.controller].exile.append(rt_obj.id)
                    else:
