...
def compile_effect(effect_text: str):
    ...
    # Fallback: try Axis2 structured parse + runtime executor when game_state is attached
    try:
        from axis3.translate.compilers.axis2_bridge import compile_and_execute_text

        gs = getattr(compile_effect, "_game_state", None)
        if gs is not None:
            compile_and_execute_text(gs, text, "bridge", 0)
